...
export function NoteEditor({ content, onChange, isEditing, setIsEditing }: NoteEditorProps) {
    ...
    return (
      <div className="max-w-[800px] mx-auto w-full h-full">
        <textarea
          ...
          className="w-full h-full min-h-[500px] p-4 font-mono text-sm bg-transparent border-none resize-none focus:ring-0"
  ...
  return (
    <div 
      className="max-w-[800px] mx-auto w-full pb-32 cursor-text"
...
function InternalLink({ name, tasks }: { name: string, tasks: any[] }) {
  ...
  // Try to find a linked task or note
  ...
  return (
    <span 
      ...
      onClick={(e) => {
        e.stopPropagation();
        // Here we could open the task modal or navigate to the note
      }}
    >
        ...
        >
          {linkedTask ? (
            <>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-zinc-100">
                <div className="flex items-center text-xs text-zinc-500">
                  ...
                  <input 
                    type="date" 
                    className="border-none bg-transparent p-0 text-xs text-zinc-500 focus:ring-0 cursor-pointer w-[100px]"
                    value={linkedTask.dueDate ? new Date(linkedTask.dueDate).toISOString().split('T')[0] : ''}
                    onChange={(e) => updateTask(linkedTask.id, { dueDate: e.target.value ? new Date(e.target.value).getTime() : undefined })}
                  />
                </div>
                  ...
                  {['low', 'medium', 'high'].map(p => (
                    <button
                      ...
                      className={cn(
                        ...
                        linkedTask.priority === p ? 
                          (p === 'high' ? 'bg-red-500 border-red-500' : p === 'medium' ? 'bg-amber-500 border-amber-500' : 'bg-blue-500 border-blue-500') : 
                          'bg-white border-zinc-300'
                      )}
                      onClick={() => updateTask(linkedTask.id, { priority: p as any })}
            ...
            <>
              ...
              {linkedNote.content && (
                <p className="text-xs text-zinc-500 line-clamp-3 overflow-hidden text-ellipsis whitespace-pre-wrap">
                  {linkedNote.content.replace(/^---\n[\s\S]*?\n---\n/, '').slice(0, 150)}...
                </p>
              )}
            </>
          ) : null}
...
function InlineProperty({ propKey, propValue }: { propKey: string, propValue: string }) {
  ...
  return (
    <span className={cn(
      "inline-flex items-center px-1.5 py-0.5 mx-1 rounded-md text-[11px] font-medium border cursor-pointer hover:opacity-80 transition-opacity",
      colorClass
    )}>
