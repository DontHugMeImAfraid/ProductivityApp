  ...
  if (!currentProject) {
    return (
      <div className="p-6 h-full flex flex-col">
        ...
        {showProjectPanel && (
          <ProjectPanelOverlay onClose={() => setShowProjectPanel(false)} />
        )}
      </div>
    );
  }
    ...
    <div className="p-4 md:p-6 h-full flex flex-col gap-4">
      ...
      {showProjectPanel && (
        <div className="fixed inset-0 z-40 flex">
          <div className="absolute inset-0 bg-black/20" onClick={() => setShowProjectPanel(false)} />
          <div className="relative ml-0 w-80 bg-white border-r border-slate-200 h-full shadow-2xl p-5 overflow-y-auto animate-in slide-in-from-left duration-250">
            <ProjectSelector onClose={() => setShowProjectPanel(false)} />
          </div>
        </div>
      )}
    ...
    return (
      <div className="space-y-3 overflow-y-auto h-full pb-4">
        {statuses.map(status => {
          ...
          return (
            <div key={status} className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50/50">
                <div className="flex items-center gap-2.5">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: settings.statusColors[status] ?? '#94a3b8' }} />
              ...
              <div className="divide-y divide-slate-50">
                {statusTasks.map(task => (
                  ...
                  >
                    ...
                    />
                    <span className={cn("text-sm text-slate-800 flex-1", task.status === 'Done' && "line-through text-slate-400")}>
                      {task.title}
                    </span>
                    {task.dueDate && (
                ...
                {statusTasks.length === 0 && (
                  <p className="px-4 py-3 text-xs text-slate-400">No tasks</p>
                )}
              </div>
            </div>
          );
